...
def combinations(nums: list[int], target: int) -> list[list[int]]:
  return combinations_helper(nums, target, [])
...
def test_combinations_2_elem_without_1():
  ...
  (3,2)
  """
  assert combinations([2, 3], 5)==[[2, 3], [3, 2]]
...
def test_combinations_2_elem():
  ...
  (2,1)
  """
  assert combinations([1, 2], 3)==[[1, 1, 1], [1, 2], [2, 1]]

def test_combinations_2_elem_ko():
  assert combinations([2, 4], 5)==[]
